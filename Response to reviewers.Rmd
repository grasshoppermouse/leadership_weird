...
We did not have any predictions.
...
Good suggestion. We now report several analyses of text records that had evidence for high status vs. those that did not. See above for details.
...
We now are careful to frame our results as "evidence for" or "lack of evidence for" leadership traits in each culture, and to make clear that absence of evidence is not evidence of absence.
...
We appreciate this point, however as mentioned above, we did not incorporate proper phylogeny controls. The PSF as a stratified random sample provide some safeguards against effects of non-independence, although not perfectly. We do now include significant additional analyses reporting region and subsistence effects as control measures. We also now report the GLMM for each of our researcher-coded variables which includes fixed effects for publication date and random effects for author nested within culture. 
...
In the SI we now include a section with additional analyses on our "Feature models." One feature of leader qualities, which was extracted from the updated cluster analysis results, is "Dominance," which includes the "coercive authority" variable. In response to the reviewers comments we provide a correlation plot illustrating the relationship between these features at the text-record level. Correlations between features are modest, but interestingly the "Organize", "Prosociality", and "Mediate" features -- each of the leader functions features -- correlate positively with the "Dominance" leader qualities features; these are among the strongest inter-feature correlations, which in our view supports the perspective that the dominance-style of leadership is closely linked with the emergence of leadership in collective action and conflict resolution. We now discuss this result in the MS. 

NEED TO DO. Add to MS. 
