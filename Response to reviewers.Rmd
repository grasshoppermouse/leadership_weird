...
We now additionally create "feature" variables based on the clusters we did find, and investigate their relationship to culture-level variables. As before, we find little convincing evidence of meaningful patterns, which we now briefly report. Two feature variables -- Mediate and Big man -- were significantly associated with region and subsistence strategy, as we now report in the SI.
...
We have substantially revised the mansucript and analyses regarding this point. We now include specific sections in the results, discussion, and SI which are focused on the relaitonship between leadership and high social statuts. 
...
We now include a paragraph in our conlcuding statement interpreting our overall findings in reference to Wofford's and GLOBE study results. 
...
We explicity avoided making any predictions. We understand that our review of existing evidence and theory could be used to generate a number of predictions. But ultimatley, our empirical goals are simply to explore the data structure rather than test predictions. 
...
We appreciate this point, however as mentioned above, we did not incorporate proper phylogeny controls. The PSF as a stratified random sample provide some safeguards against effects of non-independence, although not perfectly. We do now include significant additional analyses more systematically including and reporting region and subsistence effects as control measures. We also now report the GLMM for each of our researcher-coded variables which includes random effects for author nested within culture and we report on possible sources of biases in the SI.
