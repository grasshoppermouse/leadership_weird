...
Our third goal was to assess the extent to which each of the leadership trait and cost/benefit variables varied by four theoretically important factors for which we had data for all rows: subsistence strategy, continental region, group context, and leader sex.
...
Our fifth goal was to assess variation in our "feature" dimensions by subsistence strategy, continental region, group context, and leader sex.
...
```{r heatmapregion, fig.cap="Heatmap of relative estimated marginal means of the levels of evidence for each outcome variable by each continental region. Estimated marginal means were standardized by rows. The colors therefore represent the relative amount of evidence within the same outcome variable."}

plot(p_heatmap_region)
```


```{r heatmapsex, fig.cap="Heatmap of relative estimated marginal means of the levels of evidence for each outcome variable by leader sex. Estimated marginal means were standardized by rows. The colors therefore represent the relative amount of evidence within the same outcome variable."}

plot(p_heatmap_sex)
```

```{r heatmapgroups, fig.cap="Heatmap of relative estimated marginal means of the levels of evidence for each outcome variable by each group context. Estimated marginal means were standardized by rows. The colors therefore represent the relative amount of evidence within the same outcome variable."}
...
We constructed `r length(features)` "feature" variables from the higher-level clusters (Figure \@ref(fig:cluster)): `r paste(features, collapse = ', ')`. The Mediate, Prosociality, and Organize features were modestly positively correlated with each other, but not with Prestige or Social/material success. Prosocial competencies was correlated with Cultural conformity. See Figure \@ref(fig:featurecor).
