...
There were `r vary_n` models of leadership dimensions with an improvement relative to an intercept-only model, by AIC. Inspection of these models showed that these dimensions varied primarily by group context, but not by subsistence strategy, continental region, or leader sex. See Figure \@ref(fig:heatmaptopcontext). The one exception was our most common variable, *High status*, which varied by multiple factors, and which we examine in detail below.

```{r heatmaptopcontext, fig.cap="Leadership dimensions that were common in the ethnographic record, but varied by group context. Colors represent the relative estimated marginal means of specific group context. Means were standardized by rows, and therefore can only be compared within rows."}
heatmap_top_context
```

We then created heatmaps that displayed the relative estimated marginal means for each level of these categorical explanatory variables for each outcome variable. See Figures S\@ref(fig:heatmapsubsis), S\@ref(fig:heatmapregion), S\@ref(fig:heatmapsex), and S\@ref(fig:heatmapgroups), and Tables S\@ref(tab:variablevars) and S\@ref(tab:universalvars).
...
## When do leaders also have high status? {highstatus}
