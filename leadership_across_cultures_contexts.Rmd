...
Subsistence strategy was confounded with continental region. African text records were predominantly from horticultural societies, for instance, whereas North American text records were predominantly from hunter-gatherer societies (see Supplimentary Figure \@ref(fig:regionsubsistence)).
...
We then explored relationships between total leader benefits and costs, summed across all leader benefit and cost variables, respectively, and levels of group context. In general, leaders at the highest supracommunity political level had the most evidence for receiving benefits and the least evidence for suffering costs and leaders of military groups has the highest total costs. See Figure \@ref(fig:leadertotalbenefitscosts).

```{r leadertotalbenefitscosts, fig.cap="Estimated marginal means of A. leader total benefits, and B. total costs, as a function of group structure, based on a logistic mixed effects regression model with random intercepts for ethnographer nested within culture. Values are on the response scale."}
p1 <- 
...
```{r FunctionRE, fig.width=10, fig.cap="Ethnographer and Culture random effects for function variables."}
plot_fun_auth_cult_re
```

```{r QualityRE, fig.width=10, fig.cap="Ethnographer and Culture random effects for quality variables."}
