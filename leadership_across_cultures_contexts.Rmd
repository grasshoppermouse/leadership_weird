...
Below we plot the estimated margianl means of the effects for level of evidence for each of the `r vary_n` (out of `r nrow(multi_allvars)` total) which produced a model that showed an improvement *with* the four factor predictors over an intercepet only model, by the variable which produced the model improvement. See Figure \@ref(fig:heatmapdrop1)A in the main text. Figure \@ref(fig:heatmapgroups) plots the ledaerhsip dimensions which varied by group structure, Figure \@ref(fig:heatmapsubsis) plots the leadership dimensions which varied by subsistence stragegy, Figure \@ref(fig:heatmapregion) plots the leadership dimnenbsions which varied by contintental region, and Figure \@ref(fig:heatmapsex) plots the leadership dimensions which variaed by leader sex. Table \@ref(tab:variablevars) displays the nbumber of text records providing evidence for each of these the `r vary_n` varying leaderhsip dimennsions and the results of logistic mexed effects models including the AIC scores for the intercept only models, AIC values for models with the four predictors, the AIC difference, and adjusted p-values for the four factor coefficents. Table \@ref(tab:universalvars) displays the same information for the remaining leaderhsip dimensions which did not substantially vary by our four factors. 
