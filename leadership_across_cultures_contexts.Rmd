...
<!-- CLOSING -->
We first briefly review the qualities of leaders, the functions they serve, and the costs and benefits of leadership across nonindustrial, non-WEIRD human societies. We then outline the aims and methods of the current cross-cultural study -- which focuses on exploring patterns among non-WEIRD populations -- before presenting results and discussion.

## The functions of leadership across cultures and contexts
<!-- INTRO -->
@roscoe_social_2009, drawing on the ethnography of highland New Guinea, proposes that leadership in small-scale societies is structured by the joint demands of facilitating within-group conflict resolution and cooperation, and signaling military formidability to rival groups. Among many ethnographically described small-scale societies, which generally lack institutionalized political structures, leadership at the community level is typically informal and ephemeral and leaders emerge contextually when group demands require a managerial presence [@von_rueden_leadership_2015; @kantner_identifying_2010; @lewis_leaders_1974; @boehm_egalitarian_1993]. Theoretical literature suggests these demands most commonly arise in the contexts of collective actions, resolving conflicts between families, and punishing norm violations [e.g., @glowacki_leadership_2015; @gavrilets_convergence_2016; @hooper_theory_2010; @von_rueden_solving_2015; @henrich_big_2015; @pietraszewski_evolution_2019]. Leaders also play important roles in ritual contexts, such as in initiations [@singh_cultural_2017], facilitating marriages [@walker_evolutionary_2011], and organizing feasts [@wiessner_food_1996]. Between-group interactions, including both conflict and cooperation, are also common among hunter-gatherers and other nonindustrial societies [@glowacki_evolutionary_2017; @hames_pacifying_2019; @richerson_cultural_2016; @henrich_evolution_2009; @apicella_social_2012]. Representing the group and managing internal and external relationships and conflicts are expected to be the most salient functions of smaller-scale community leaders. 
...
Defensibility of critical resources creates increased opportunities for resource management and is often associated with increased territoriality and increased pressures of inter-group conflicts [@glowacki_evolutionary_2017]. Community leaders among populations more reliant on domesticated plant foods, livestock, and specific territories are often required to manage military operations [@lopez_evolutionary_2017].
...
Evolutionary models of leadership and followership generally assume that, on average, the fitness benefits of leadership outweigh the fitness costs. However, the costs of leadership and followership have received less attention. Group responsibilities entail opportunity costs. Mediating conflicts between individuals can be physically and socially costly [@wiessner_power_2010]. In the absence of institutionalized authority, all group members are on relative equal footing and equipped to challenge leaders directly or indirectly using a variety of leveling mechanisms including physical aggression or indirect aggression, such as gossip and social ridicule [@boehm_purposive_2008; @hess_gossip_2017]. With greater population density and wealth inequality, these risks are expected to concomitantly increase in magnitude. In managing collective action and enacting group-level policies, some behaviors of leaders will upset some followers and leader-follower interactions can be hostile. Physically formidable individuals with strong social networks are likely to be able to bear the costs of leadership more so than their counterparts [@glowacki_leadership_2015]. 

<!-- BENEFITS: DECISIONS IN THEIR INTEREST -->
Leadership positions, including formal and informal roles, are also commonly associated with numerous social, material, and reproductive benefits [@von_rueden_roots_2014; @von_rueden_multiple_2008; @grammer_symptoms_1996; @cheng_psychology_2014]. Leaders and high-status individuals are more likely to be the recipients of social support and material benefits in times of need [@gurven_its_2000; @von_rueden_roots_2014; @sugiyama_illness_2004]. Across many WEIRD organizations leadership positions are associated with increased financial compensation [@tsui_income_2017]. High social status, material wealth, intelligence, and leadership status are also generally viewed as sexually attractive, particularly to women [@buss_strategies_2006; @stanik_who_2010]. Most measures of reproductive success are positively associated with male social status, independent of subsistence type or status measure [@von_rueden_mens_2016].
...
We analyzed data from our operationalized dimensions at the text-record level and at the culture-level. Our first goal was to assess the proportion of text records and cultures that provided evidence for each of our coded dimensions to discover the qualities, functions, benefits and costs of leadership that are most and least frequently noted in the ethnographic record. We computed the proportion of text records supporting each dimension, and their 95% confidence intervals, using logistic mixed effects models with random intercepts for document authors nested within cultures (which partially accounts for potential variation due to differences in ethnographers such as gender, nationality, or theoretical framework). For each dimension, we then computed the proportion of cultures with at least one text record with evidence for that dimension, with confidence intervals computed using a cluster bootstrap.

Our second goal was to determine which leader and follower benefits and costs were most commonly mentioned in the ethnographic record, computing proportions and 95% CIs as above and to estimate how costs and benefits varied by group structure types. Our third goal was to assess the extent to which each of the researcher-coded dimensions of leadership varied by four theoretically important factors for which we had data for all rows: group context, subsistence strategy, continental region, and leader sex. We also performed textual analyses and explored variation measures of biases stemming from the ethnographic record that may have impacted results.

Our fourth goal was to explore lower dimensional representations of these data that might reveal informative "features." These included logistic PCA, non-negative matrix factorization (NMF), and hierarchical cluster analysis. See the SI for brief descriptions of these methods. Our fifth goal was to assess variation in our lower-dimensional "features" by group context, subsistence strategy, continental region, and leader sex. Our sixth goal was to evaluate the features in light of influential evolutionary theories of leadership. Drawing on the empirically derived features as a guide, we aimed to construct "elements" of leadership that corresponded to elements of influential theories.
...
```{r highstatusplot, fig.width=10, fig.cap="Coefficients of logistic elasticnet regression models [@friedman_regularization_2010] of evidence for *High Status*. A: Coefficients indicate the words whose frequencies in each text record best predicted evidence for *High status* in each text record. B: Non-zero coefficients of all quality and function dimensions that best predicted evidence for *High status*. Both models used the lasso penality ($\\alpha=1$), with $\\lambda_{1se}$ chosen by cross-validation (1 SE from $\\lambda_{min}$). Red values are positive predictors; purple values are negative predictors. Predictors with coefficents = 0 not displayed. X-axes are odds ratios on a log 10 scale."}
...
The leader quality cluster analysis (Figure \@ref(fig:cluster)A) identified two top-level clusters, one that comprised mostly dimensions related to prosocial qualities and one that comprised other individual-level qualities which were not necessarily prosocial. Within the latter top-level cluster we identified two interpretable and strongly supported subclusters which are annotated with text (*Social, reproductive, and material success* and *Competencies*). Within the first top-level cluster, we also identified two subclusters, one of which contained measures related to group-level pressures (*Cultural conformity*) and one which contained the prosocial traits (*Prosocial competencies*); these subclusters were moderately supported.

The leader functions cluster analysis (Figure \@ref(fig:cluster)B) also identified two top-level clusters; the larger of which we identified as a management functions cluster (e.g., *Organize cooperation*, *Military command*, *Resolve conflict*) and the smaller of which we identified as a prosociality cluster (e.g., *Social functions*, *Distribute resources*, *Hospitality*). Within the management cluster, we identified two moderately supported subclusters; a *Strategize* subcluster (e.g., *Resolve conflict*, *Provide counsel*), and an *Organize* subcluster (e.g., *Organize cooperation*, *Military command*).

We identify these seven named subclusters as "features" of leader qualities and functions in the ethnographic record. In naming these features, we gave extra weight to dimensions that were strongly supported in the sample (Figure \@ref(fig:varsupport)), and therefore not all dimensions within these clusters were clearly related to the themes of each feature. These features correspond fairly well to major elements of evolutionary theories of leadership reviewed in the introduction, with the possible exception of competencies, a single feature that included at least two elements (prestige and dominance). Because features were aggregated dimensions, they are more common at both the text record and culture level (see Figure \@ref(fig:featuressupportplot)).
...
```{r capital}
capital_pct <- signif(100*sum(rowSums(df_fivefold2[1:5])>0)/nrow(df_fivefold2), 2)
```


Motivated by our feature analysis, and the apparent importance of shaman leaders, we tentatively advance a scheme, similar to @von_rueden_roots_2014[] [and inspired by @kaplan_embodied_2003; @kaplan_evolutionary_2009; @borgerhoff_mulder_intergenerational_2009], whereby leaders possess some combination of cognitive capital, social capital, and somatic capital that they deploy to provide benefits and impose costs (for further justification, see the Discussion section). Hence, we should be able to find examples of leaders with various combinations of these five elements.^[We omit material capital because this could be both a means to leadership as well as a benefit of leadership.]

To test this idea, we operationalized the five elements using the leadership qualities cluster analysis (Figure \@ref(fig:cluster)A) as guidance (see Table \@ref(tab:leadermarket)). About half of the text records (`r capital_pct`%) had evidence for one or more of the five elements, in heterogeneous combinations. See Figure \@ref(fig:fivefold). In particular, not all leaders have evidence for high *Social capital*, leaders with *Cognitive capital* provide benefits *and* impose costs, and *Somatic capital* is rarely mentioned, but when it is, it is associated with providing benefits as well as imposing costs. Shamans are associated with evidence for *Cognitive capital* and imposing costs.

```{r fivefold, fig.width=10, fig.cap="The intersections of evidence for our five basic elements of leadership among all text records (an 'upset' plot) [@gehlenborg2019]. Horizontal bars (left): The total number of text records in each element. Black dots: has evidence. Gray dots: does not have evidence. Vertical bars (top): The number of text records in that intersection (with lower limit set at 3). Red: The number of text records involving shamans or leaders with supernatural qualities."}
...
We assessed the benefits and costs of leadership in two ways. First, we explicitly coded each text record for evidence of benefits and costs. Leaders were widely reported to receive material benefits, social status, and mating benefits but loss of material resources and social status were also widely reported, suggesting leadership to be a high-risk, high-return strategy (see Figure \@ref(fig:varsupportcb)). Nevertheless, for most dimensions of benefits and costs, there was more evidence of benefits than costs (Figure \@ref(fig:benefitcostratio)), and the same was true averaging across all benefit/cost dimensions ($OR=`r signif(leader_bc_ratio$benefit_cost_OR, 2)`$). Second, several positive leader qualities indicating social, reproductive, and material success clustered together (bottom cluster Figure \@ref(fig:cluster)A). Both results, while not providing evidence of net fitness benefits, are nevertheless encouraging, especially since leaders had more evidence of mating and territory benefits than costs, which are key contributors to fitness, and also had more evidence for inclusive fitness, material and social benefits, relative to costs. This is consistent with much of the theoretical literature that links leader benefits to their capacities to provide prosocial benefits (e.g., resolving conflicts, organizing collective actions, and being generous) and/or impose social costs (e.g., punishing and aggressiveness) [e.g., @von_rueden_mens_2016; @bowles_emergence_2010; @glowacki_role_2013; @hooper_theory_2010; @gavrilets_solution_2014; @price_evolution_2014; @smith_leadership_2016].
...
Relationships between leadership and high status, including causality and dependence, are puzzling and currently the focus of much work in evolutionary leadership studies [e.g., @van_vugt_dual_2019; @smith_leadership_2020; @cheng_why_2020; @cheng_dominance_2019; @von_rueden_dynamics_2019; @garfield_evolutionary_2019]. Drawing on a comparative-evolutionary perspective, @van_vugt_dual_2019 emphasize that although leadership and high social status do often covary, leaders are not always high-status individuals, exemplified by low-ranking individuals in nonhuman species that lead group movement or younger and lower status men and women who initiate collective actions. @cheng_dominance_2019 emphasizes, however, that among humans high social status may most often be a necessary but insufficient condition for achieving and maintaining positions of leadership [see also @cheng_why_2020]. The "low status" young adults who initiate a collective action might be high status among their peers, within what @garfield_review_2019 [p. 71] described as the "intragenerational social hierarchy."
...
Leadership and social influence are expected to be associated with particular suites of individual qualities and functions, which potentially covary by cultural or social context, personality, or other inter-individual variation [@cheng_two_2013; @von_rueden_roots_2014; @hogan_personality_2020; @lukaszewski_extraversion_2015; @henrich_evolution_2001; @bass_handbook_1990]. Our systematic cross-cultural results support many aspects of influential evolutionary theories of leadership, but offer new direction for the expansion and synthesis of leadership theories. 

**Dominance and prestige**. Much of the evolutionary theoretical literature on leadership has suggested social influence is most often achieved via one of two distinct strategies: dominance or prestige [for review see @garfield_review_2019; @garfield_evolutionary_2019]. There is a long history of leadership styles being described as based on either force and strength (i.e., dominance) or respect and special skills (i.e., prestige). Such dual-models are rooted in the coupling of ethology and animal dominance hierarchies with ethnography and cultural prestige systems [see @mead_sex_1935; @sahlins_poor_1963; @lewis_leaders_1974; @tiger_dominance_1970; @bernstein_dominance_1976; @barkow_prestige_1975]. Synthesizing the ethnographic and ethological evidence, @kracke_force_1978 linked "force and persuasion" to psychological-affect between leaders and followers in small-scale societies and Barkow [-@barkow_prestige_1975; -@barkow_prestige_1980] suggested the evolution of human prestige was linked to intra- and inter-sexual selection of culturally acquired competencies. 

@henrich_evolution_2001 introduced this important distinction to social science disciplines outside of anthropology, agreeing with @tiger_dominance_1970 and Barkow [-@barkow_prestige_1975; -@barkow_prestige_1980] on the evolutionary roots of dominance, but offering an evolutionary account of prestige rooted in social rather than sexual selection for culturally acquired competencies. @henrich_evolution_2001 inspired much subsequent work across the behavioral sciences [e.g., @van_vugt_dual_2019; @cheng_two_2013; @maner_2016; @halevy_status_2012; @suessenbach_dominance_2019; @maner_dominance_2017; @garfield_evolutionary_2019; @garfield_investigating_2019]. @van_vugt_dual_2019 [p. 1], for example, "argue that there are two leadership styles: prestige style and dominance style," and @cheng_two_2013 [p. 119] in focusing on distinctions between dominance and prestige suggests, "human social stratification is characterized by the co-occurrence of both strategies." Our analyses support the cross-cultural importance of leader aggression and coercion, as well as culturally acquired competencies, but they also indicate that some important modifications are required.

<!-- DISCUSSES COMPETENCIES FEATURE -->
**Leader competencies and shamanism**. The individual competencies cluster (Figure \@ref(fig:cluster)A) comprises dimensions associated with dominance-based leadership, including coercive authority and aggressiveness, as well as qualities associated with prestige-based leadership, including knowledgeable/intelligent and experienced/accomplished. These dimensions are among the most strongly supported in the ethnographic record (Figure \@ref(fig:varsupport)). 

Shamanism appears to be a distinct form of leadership that combines a strategy of inducing fear, similar to the dominance strategy, but is based on knowledge and expertise, similar to the prestige strategy. The *Competencies* feature included supernatural and feared dimensions (Figure \@ref(fig:cluster)). Our *Shamanism* variable (which was not one of our original leadership dimensions) was predicted by words that evoke both dominance and prestige, such as *supernatural*, *power*, *fear*, and *knowledge* (Figure \@ref(fig:shamanism)A), and by four other leadership dimensions that also overlap with the dominance and prestige constructs: *Feared*, *Medicinal functions*, *Ritual functions*, and *Experience/accomplished*. @winkelman1987cross found that in societies sampled from the HRAF ($n = 43$) shamans provided decision-making services within judiciary, economic, military, or political domains in 44% or more of societies, and held charismatic leadership roles in 19% of societies; @winkelman1992shamans emphasizes the influential positions of leadership shamans attain through their charisma, social unification, healing abilities, and use of supernatural powers to cause harm. Artistic performance is also implicated in this style of leadership and ceremonial, artistic performances by shamans are common displays which often incorporate superhuman abilities [@singh_cultural_2017]. 
...
**Leaders are managers**. There is a history within managerial studies among WEIRD populations of contrasting *leaders* from *managers*. @zaleznik_managers_1977, for example, suggested "managers" seek systematicity, organization, and stability, whereas "leaders" are willing to accept a lack of control and creatively navigate uncertainty to produce novel insight and appeal to the emotions of followers. Political anthropology reveals, however, in the absence of state-based or large-scale institutions, local leaders are often tasked with both managing various economic systems while also providing social services and interacting with followers on an interpersonal level [@cohen_comparative_1967; @fried_evolution_1967]. Within the domain of leader management, there are likely to be two primary features of human leader functions: strategizing and organizing. Our results (Figure \@ref(fig:cluster)B) correspond very closely to what @van_vugt_cognitive_2007 define as *strategic leadership*, where leaders increase the pay-offs of another individual's behavior thereby incentivizing followership, versus *coordinating leadership*, where individuals bound by group challenges must effectively organize collective effort.
...
The feature of leader organizational functions aligns with the many evolutionary theories of leadership that emphasize the role of leaders in organizing collective actions [e.g., @hooper_theory_2010; @van_vugt_cognitive_2007; @glowacki_formation_2016; @pietraszewski_evolution_2019; for review, see @garfield_evolutionary_2019]. In the ethnographic record such functions includes controlling and facilitating economic productivity and ensuring collective actions do not overly conflict with individual or household-level goals, such as among the Tukano of the Brazilian Amazon, where @sq19-018 [p. 127] notes: 
