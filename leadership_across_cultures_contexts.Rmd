...
We constructed `r length(features)` "feature" variables from the higher-level clusters (Figure \@ref(fig:cluster)): `r paste(features, collapse = ', ')`. We then fit separate logistic mixed effects models of each feature as a function of subsistence mode, with random effects for author nested within culture. After adjusting the subsistence coefficient p-values for an FDR of 0.05, we computed and plotted the estimated marginal means of the remaining significant model(s). See Figure \@ref(fig:features).

```{r features, fig.width=8, fig.cap="Esimtated marginal means [@lenth2020] of logistic mixed effects regression models of Feature variables by mode of subsistence. Only models whose subsistence coefficient had a corrected p-value < 0.10. Plotted on the response scale. See text for details."}
