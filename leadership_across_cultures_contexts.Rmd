...
Among the `r commondim_n` leader dimensions that were found in at least 60% of cultures, there were `r universals_n` that did not show evidence of varying by the four factors, and `r vary_n` that did. Because evidence for these dimensions was common across cultures, the `r universals_n` unvarying dimension are condidates for universal dimensions of leadership. See Table \@ref(tab:leaderuniversals).
...
Inspection of the `r vary_n` dimensions that varied showed that they varied primarily by group context, and not by subsistence strategy, continental region, or leader sex. See Figure \@ref(fig:heatmaptopcontext). The one exception was our most common variable, *High status*, which varied by multiple factors, and which we examine in detail below.
...
```{r heatmapdrop1, fig.cap="Heatmap of delta AIC when each predictor variable was dropped from the model. Larger values (yellow) indicate that a predictor (columns) accounted for variation in a leadership dimension (rows). Values were scaled by rows, and so can only be compared within rows."}
heatmap_drop1
```
