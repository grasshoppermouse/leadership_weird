...
Exploratory modeling revealed variation in the vast majority (INCLUDE NO. or %) of our researcher coded variables (TABLEs) did not vary by group type or subsistence strategy (adjusting for effects of leader sex and continental region and the hierarchical structure of ethnographer within culture), suggesting that many of these dimensions of leadership are potentially universal features of human social groups or at least not strongly impacted by the within or between-group measures investigated here. 
...
Our analysis identified several leader functions and qualities that were near cultural universals, and others that were more variable or relatively rare. See Figure \@ref(fig:varsupport).
...
<!-- NEED TO DO ADD SECTION OR DELETE HERE -->
...
multi_aic %>% 
  ...
  arrange(Variable) %>% 
  kable(
  caption = 'Retained logistic mixed effects models of each variable as functions of subsistence strategy, continental region, leader sex, and group structures. Models were retained due to improvement in AIC values relative to an intercept-only model. Sum: the number of rows with evidence for the variable. uniAIC: AIC of intercept-only model. multiAIC: AIC of multivariate model. AIC_diff: difference in AIC values. P-values are the unadjusted p-values of each explanatory variable in the model.',
  digits = 2
  )
```
